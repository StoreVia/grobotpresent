...
module.exports = class MessageSetAkiLang extends Command {
	...
	async run(client, message, args){
        ...
        if(!string){
            ...
            if(langCodes.includes(`${string.toLowerCase()}`)){
                await langdb.set(`${message.author.id}`, string.toLowerCase())
                return await msgdefer.edit({ content: `> Done✅. Your Akinator Language Was Now Updated.` })
            } else {
                let msg = await msgdefer.edit({ content: `> Language Code Doesn't Exist.\n**Click Below Button To View All Language Codes.**`, components: [buttonRow] })
                collector(msg);
            }
        }
